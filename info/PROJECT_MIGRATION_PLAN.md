...
## 1. Фаза 0 — Регрессионный Baseline для S_01

**Цель:** зафиксировать текущее поведение S_01 до любых изменений.

**Шаги:**

1. Выбрать небольшой baseline-дataset (кусок реальных данных или специальный CSV).
2. Написать `tools/generate_baseline_s01.py`, который:
   - запускает текущую (legacy) S_01 с фиксированными параметрами,
   - сохраняет:
     - основные метрики (Net Profit %, Max DD %, Total Trades и др.),
...
## 2. Фаза 1 — Разделение фронтенда на HTML / CSS / JS
...
4. Минимально проверить работоспособность UI.
...
### 3.2. Консолидация Optuna и удаление Grid Search
...
## 4. Фаза 3 — Пакет Индикаторов `indicators/`
...
## 5. Фаза 4 — Выделение Метрик в `metrics.py` + Первые Unit-тесты
...
7. Добавить unit-тесты:
   ...
   - `tests/test_metrics.py` — проверка `calculate_basic/advanced` на фейковом `StrategyResult`.

8. Прогнать регрессию S_01 + новые unit-тесты.

**Результат:** единственный источник правды для всех метрик — `metrics.py`, а индикаторы/метрики покрыты базовыми тестами.

---

## 6. Фаза 5 — Централизованный Экспорт в `export.py`
...
4. Прогнать регрессию S_01 + unit-тесты.

**Результат:** все экспортные задачи централизованы, форматы хранятся в одном месте.

---

## 7. Фаза 6 — Простая Стратегия и Отладка Архитектуры
...
## 8. Фаза 7 — Миграция S_01 на Новую Архитектуру (через дубль)
...
### 8.2. Создание дубля S_01 на новой архитектуре
...
### 8.3. Сравнение legacy S_01 и S_01_migrated
...
### 8.4. Переключение на новую S_01 и удаление legacy
...
## 9. Фаза 8 — Логирование, Финальная Чистка и Документация
...
## 10. Вне Текущего Scope (на будущее)
