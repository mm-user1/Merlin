...
Цель: вся основная бизнес-логика сосредоточена в трёх движках и нескольких чётких вспомогательных модулях. Стратегии и индикаторы изолированы, UI и обвязка не завязаны на внутренние детали ядра.
...
- Обход баров:
  - вызов логики стратегии (`on_bar(ctx)` или эквивалент),
...
- Выход:
  - `StrategyResult` — единственный формат результата, который потом потребляется `metrics.py`, `optuna_engine.py`, `walkforward_engine.py` и UI.
...
- Интерфейс стратегии:
  - bar-ориентированный `on_bar(ctx)` (основной путь),
  - при необходимости — векторные методы `build_indicators(df)` / `build_signals(df, params)` (можно добавить позже).
...
- реализует торговую логику в `strategy.py`;
...
Структуры данных (`TradeRecord`, `StrategyResult`, `BasicMetrics` и др.) определяются внутри соответствующих модулей и переиспользуются через импорты, без отдельного `types.py`. Простая тестовая стратегия и S_01_v2 живут в `strategies/` и используют общую архитектуру.
