...
def run_grid_search(csv_path: str, config: dict, workers: int = None, use_ha: bool = False) -> Path:
    ...
    # Используем декартово произведение вместо глубокой вложенности циклов
    ...
    _init_pool(csv_path, start_ts, end_ts, 0.0005, 100.0, 0.01, 8, use_ha)
    ...
    eta = avg * total
    print(f"ETA ≈ {fmt_dur(eta)}  (workers=1, HA={use_ha})")

    # Основной прогон
    results = []
    t0 = time.time()
    for combo in tqdm(combos, desc="Grid"):
        results.append(simulate(combo))
    dt = time.time() - t0
